...
function onRBSChangeOpenByIndex(projectId) {
	...
	if (identifier != null) {
		...
		if (result != null && result['ok'] == identifier.path) {
			...
			onChangeLogin(identifier, false);			
			return;
		}
	}
...
function openChangeLogin(ident) {
	...
	window.openDialog('chrome://rbschange/content/login.xul', 'login', 'chrome,extrachrome,resizable,centerscreen,width=650,height=340', identifier);
}
...
function onChangeLogin(identifier, cleanCache) {
	if (identifier.path) {
		...
		if (cleanCache) {
			var xchromeService = ChangeManager.getXChromeService();
	...
	try {
		...
		document.addEventListener("RBSChangeOpenDocumentEvent", function(event) { 
			...
			if (match) {
				...
				if (false && identifier && identifier.login != '' && identifier.password != '') {
					var result = ChangeManager.login(identifier.uri, identifier.login, identifier.password, identifier.uilang);
					if (result != null && result['ok'] == identifier.path) {
						ChangeToolKit.getPreferencesService().setCharPref('rbschange.ext.lastprojectid', identifier.path);
						var btn = document.getElementById('tbtnChange');
				...
				var location = elem.ownerDocument.location;
				var identifier = {baseURI: location.protocol + '//' + location.hostname};
				window.openDialog('chrome://rbschange/content/login.xul', 'login', 'chrome,extrachrome,resizable,centerscreen,width=650,height=340', identifier);				
			}
		}, true, true);
